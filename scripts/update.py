...
for topic_id, details in data.items():
    eli5_block = f'<div class="eli5"><span class="tag">In plain English</span>\n{details["eli5"]}\n</div>'
    
    # We use regex to find the block for the specific id
    pattern = r"(id:\s*'" + topic_id + r"'.*?)<div class=\"eli5\">.*?<\/div>(.*?)quiz:\{.*?\}\n\}\,"
    
    # Replacement function
    def replacer(match):
        return match.group(1) + eli5_block + match.group(2) + "quiz:" + details["quiz"] + "\n},"
        
    content = re.sub(pattern, replacer, content, flags=re.DOTALL)

with open('new_lessons.js', 'w', encoding='utf-8') as f:
    f.write(content)

print("Placeholders updated via Python.")
