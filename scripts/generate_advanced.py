advanced_lessons = [
    {
        ...
        "nav": "Service Mesh",
        "lede": "Moving routing, security, and observability out of the application using sidecar proxies.",
        "eli5": "Instead of every house (microservice) hiring its own security guard and map-reader, the city provides a standard guard and map to every house by default.",
        "full_text": """
...
""",
    ...
    },
    {
        ...
        "lede": "How to handle payments across microservices without distributed locks.",
        "eli5": "Instead of freezing everyone's bank account until a transfer is perfectly complete, we just keep a ledger of promises. If a promise breaks, we write a refund entry to cancel it out.",
        "full_text": """
...
""",
    ...
    },
    {
        ...
        "lede": "The backbone of modern AI/LLM applications and semantic search at scale.",
        "eli5": "Instead of finding exact word matches in a dictionary, we map words into a 3D galaxy. Words with similar meanings cluster together, so we just look for the closest stars.",
        "full_text": """
<h3>Embeddings</h3>
...
""",
    ...
    },
    {
        ...
        "lede": "How nodes elect leaders and prevent split-brain using Raft and Paxos.",
        "eli5": "In a room of 5 people, nobody can make a rule unless at least 3 people vote for it. If the room is split in half by a wall, neither side has 3 people, so no bad rules are made.",
        "full_text": """
...
""",
    ...
    },
    {
        ...
        "nav": "CRDTs",
        "lede": "Conflict-free Replicated Data Types for lockless simultaneous editing.",
        "eli5": "Instead of waiting in line to write on a whiteboard, everyone has their own whiteboard. Whenever you draw something, you shout out what you drew, and everyone automatically merges it perfectly without erasing each other's work.",
        "full_text": """
...
for lesson in advanced_lessons:
    ...
    nav = lesson['nav']
    lede = lesson['lede'].replace("'", "\\'")
    
    html = f"""
<p class="eyebrow">{lesson['group']} &middot; Lesson {lesson['id']}</p>
<h1>{lesson['title']}</h1>
...
"""
    # Replace backticks in HTML to prevent breaking JS template literals
    html = html.replace('`', '\\`')
...
with open('advanced_lessons.js', 'w', encoding='utf-8') as f:
    f.write(js_code)

print("Created advanced_lessons.js!")
